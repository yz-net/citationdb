...
let res = await fetch(
  "https://api.github.com/repos/yale-fortunoff/citationdb-staging/actions/workflows",
);
let { workflows } = await res.json();
const workflow = workflows.find(
  (w) => w.path.indexOf("build-from-airtable") > -1,
);

console.log("Found workflow:", workflow.id, workflow.name);

const dispatchRes = await fetch(
  `https://api.github.com/repos/yale-fortunoff/citationdb-staging/actions/workflows/${workflow.id}/dispatches`,
  {
    method: "POST",
    headers: {
